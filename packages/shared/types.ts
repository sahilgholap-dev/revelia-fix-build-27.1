...
// Used by both mobile and server
...
export interface BirthLocation {
  city: string;
  country: string;
  // lat/lng/timezone populated by server geocoder. Optional + nullable to
  // accommodate legacy mobile sends (lat:0/lng:0 placeholders) and the
  // unresolvable-place case (server saves text without coords).
  lat?: number | null;
  lng?: number | null;
...
export interface BirthData {
  ...
  // True when server defaulted a missing birth time to noon at the
  // place-of-birth timezone. Mobile uses this to render moon/rising values
  // with a small (i) provenance indicator.
...
export interface UploadResponse {
  ...
  type: ImageType;
  uploadedAt: string;
  // Build 21+: validation status accompanies successful upload responses.
  // 'valid' = proceed silently. 'uncertain' = mobile shows soft-fail modal
  // (retake or continue). 'invalid' is thrown as 422, never appears here.
  validation?: {
...
// ============================================================================
// The ONE small persisted field on the profile — the last-engagement timestamp
// the current temporal delta is measured FROM, plus the algorithm version tag.
// Dual-homed to mirror NatalChart/NumerologyNumbers. The COMPUTED delta type
// (ContinuityDelta) is deliberately server-only (never a DTO, never persisted)
// and lives in server/src/types/shared.ts. See plans/build-27/R6-continuity.md §5.
